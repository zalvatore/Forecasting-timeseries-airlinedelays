...
arr_delays_pit.ts <- ts(Airline_Delays_Pit_sum$Freq  , ## arr_delay is the total amount of delay in minutes
                 start = c(2019, 1),
                 end = c(2022, 8),
                 freq = 12)
...
autoplot(Airline_Delays_Pit.train, series = 'Train') +
  autolayer(Airline_Delays_Pit.test, series = 'Valid')
```
## Modeling
...
ets.pred <- forecast(wine.hwin, h=forecast.periods)
...
#first difference
y %>% 
  ggtsdisplay(main="Inital Time Series")

#first  seasona difference
y %>% 
  diff(lag = 12) %>% 
  ggtsdisplay(main="First Seasonal Difference")

#first difference
y %>% 
  diff(lag = 12) %>% 
  diff() %>% 
  ggtsdisplay(main="One Season Diff, One Non Season Diff")
...
#Arima
fit <- arima(y , order=c(0,1,1), seasonal = c(1,1,1))
summary(fit)
...
predictors <- as.matrix(y.frame.new[c("Covid")] )
predictors.valid <- as.matrix(test.y.frame.new[c("Covid")] )
