...
wine.hwin <- ets(Airline_Delays_Pit.train,
                 model = "AAM",
                 alpha = .2,
                 gamma = .05,
                 restrict = FALSE
...
#first difference
Airline_Delays_Pit.train.ts %>% 
  diff() %>% 
  ggtsdisplay(main="First Difference")

#Autoarima
auto_fit <- auto.arima(Airline_Delays_Pit.train.ts)
summary(auto_fit)

#Arima
fit <- arima(Airline_Delays_Pit.train.ts, order=c(3,1,0))
summary(fit)
...
#Forcast
forecast(auto_fit, h = 6)

#Plot
autoplot(forecast(auto_fit, h = 6))
